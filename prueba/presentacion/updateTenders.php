...
set_time_limit(400);
        ...
        $url = "http://api.mercadopublico.cl/servicios/v1/publico/licitaciones.json?fecha=11082020&ticket=83D4D082-755A-41DC-8B28-1DDA010F765E";
                            ...
                            foreach ($listado2 as $idx3 => $listado3) {
                                if ($idx3=="Items" && $listado3!=null) {
                                    foreach ($listado3->Listado as $key => $value) {
                                        $sql = "INSERT INTO detalle_listado (cod_listado,cod_producto,cantidad) VALUES (".$row['cod_listado'].",$value->CodigoProducto,".$value->Cantidad.")";
                                        $insert = mysqli_query($conexion, $sql);
                                        $sql2 = "INSERT INTO producto (cod_producto,nom_producto,cod_categoria) VALUES ($value->CodigoProducto,'$value->NombreProducto',$value->CodigoCategoria)";
                                        $insert2 = mysqli_query($conexion, $sql2);
